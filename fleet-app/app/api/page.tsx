...
import {
  ...
  getJarak, getTollTarif, hitungEstBbm, fmtRupiah, fmtTgl, fmtTglShort, today,
} from '@/lib/data';
import {
  appendTrip, appendRencana, getData, getRencana, getStats, updateStatus,
  RencanaData, TripData,
} from '@/lib/gasApi';
import {
...
function EstBox({ bbm, toll }: { bbm?: number; toll?: number }) {
  if (!bbm && !toll) return null;
...
function TabMonitor() {
  ...
  useEffect(() => {
    Promise.all([getRencana(), getStats()]).then(([r, s]) => {
  ...
  function armadaStatus(name: string) {
    const trip = aktifHariIni.find(r => r.armadaName === name);
    return trip
      ? { label: 'Dipakai', color: 'amber' as const, info: `${trip.driver} · ${trip.tujuan || trip.lokasiTujuan || trip.kategori}`, border: 'var(--amber)' }
      ...
      <Card>
        <div style={{ display: 'flex', gap: 0, flexWrap: 'wrap' }}>
          {[
            ...
            { label: 'Aktif hari ini', val: String(aktifHariIni.length) },
          ].map((s, i) => (
            ...
            {aktifHariIni.map((r, i) => (
              <div key={i} style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>{r.driver} · {r.jamMulai} · {r.tujuan || r.lokasiTujuan || r.kategori}</div>
                </div>
            ...
            {rencanas.slice().reverse().map((r, i) => (
              <div key={i} style={{
                ...
                <div style={{ display: 'flex', gap: 5, flexShrink: 0 }}>
                  <Btn size="sm" variant="success" onClick={() => ubahStatus(r.id ?? i + 2, 'Jadi')}>✓ Jadi</Btn>
                  <Btn size="sm" variant="danger"  onClick={() => ubahStatus(r.id ?? i + 2, 'Batal')}>✕</Btn>
                </div>
              </div>
            ))}
          </div>
...
function TabRealisasi({ setToast }: { setToast: (s: string) => void }) {
  ...
  function pilihRencana(id: number) {
    ...
    if (r) setForm({
      ...
      kmAwal: r.kmAwal,
      // Auto-fill estimasi dari rencana
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <Card>
        ...
        {!rencanas.length ? (
          ...
          <Select label="Pilih Rencana yang Jadi" value={selectedId ?? ''} onChange={e => pilihRencana(Number(e.target.value))}>
